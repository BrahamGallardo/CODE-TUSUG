...
begin	
	if NEW.puesto = 'secretaria' OR NEW.puesto = 'mantenimiento' OR NEW.puesto = 'recursos humanos' OR NEW.puesto = 'almacen' then
		select id_rol into idPuesto from sistemaTusug.rol where descripcion like NEW.puesto;
		insert into sistemaTusug.usuario values(NEW.rfc, idPuesto, MD5(NEW.rfc), NEW.url_img, current_date);
	else return null;
	end if;
	return new;
