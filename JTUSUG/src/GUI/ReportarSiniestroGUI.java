    ...
    class ReportCustomListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            switch (op) {
                ...
                case "guardar":
                    try {
                        if (validaIngreso(reclama, paga) && calendarioSiniestro.getDate() != null
                                && calendarioIngreso.getDate() != null
                                && calendarioIndemnizado.getDate() != null) {
                            java.util.Date siniestro = new java.util.Date(calendarioSiniestro.getDate().getTime());
                            java.util.Date ingreso = new java.util.Date(calendarioIngreso.getDate().getTime());
                            java.util.Date indemnizado = new java.util.Date(calendarioIndemnizado.getDate().getTime());
                            if (siniestro.getTime() < ingreso.getTime() && ingreso.getTime() < indemnizado.getTime()) {
                                controlador.agregaMan();
                                controlador.creaRepor();
                            } else {
                                JOptionPane.showMessageDialog(null, "Orden de fechas invalido: Debe ser siniestro<ingreso<indmnizado", "Error..!!", JOptionPane.ERROR_MESSAGE);
                            }
                        } else {
